...
class Cell:
    def __init__(self, num_cells):
        if isinstance(num_cells, int):
            if num_cells <= 0:
                raise ValueError('Параметр num_cells должен быть больше нуля')
            self.num_cells = num_cells
        else:
            raise TypeError('Параметр num_cells должен быть int')
    ...
    def __truediv__(self, other):
        result = int(round(self.num_cells / other.num_cells, 0))
        return result

    def make_order(self, rows):
        number, remainder = divmod(self.num_cells, rows)
        if number == 0:
            return f'Количество рядов превышает количество ячеек'
        else:
            stroka = ''
            for i in range(rows):
                st = f'{"*" * number}\n'
...
print('Сумма: ', a + b)
print('Вычитание >0: ', a - b)
print('Вычитание <0: ', b - a)
print('Умножение: ', a * b)
print('Деление: ', a / b)

print(f'Организация ячеек по рядам вар1:\n{a.make_order(2)}')
print(f'Организация ячеек по рядам вар2:\n{b.make_order(9)}')
